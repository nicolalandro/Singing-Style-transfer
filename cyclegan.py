...
class CycleGAN(object):
    def __init__(self, num_features, g_type="gated_cnn", discriminator=discriminator, generator=generator,
                 generator_unet=generator_unet, mode='train', log_dir='./'):
    ...
    def build_model(self):
        ...
        self.discriminator_loss_A_real = l2_loss(tf.ones_like(self.discrimination_input_A_real),
                                                 self.discrimination_input_A_real)
        self.discriminator_loss_A_fake = l2_loss(tf.zeros_like(self.discrimination_input_A_fake),
                                                 self.discrimination_input_A_fake)
        self.discriminator_loss_B_real = l2_loss(tf.ones_like(self.discrimination_input_B_real),
                                                 self.discrimination_input_B_real)
        self.discriminator_loss_B_fake = l2_loss(tf.zeros_like(self.discrimination_input_B_fake),
    ...
    def optimizer_initializer(self):
        ...
        self.generator_optimizer = tf.train.AdamOptimizer(self.generator_lr, beta1=0.5).minimize(self.generator_loss,
                                                                                                 var_list=self.generator_vars)
        self.discriminator_optimizer = tf.train.AdamOptimizer(self.discriminator_lr, beta1=0.5).minimize(
    ...
    def train(self, input_A, input_B, cycle_lambda, identity_lambda, generator_lr, discriminator_lr):
        ...
        generation_A, generation_B, generator_loss, generator_summaries, _ = self.sess.run(
            [self.generation_A, self.generation_B, self.generator_loss, self.generator_summaries,
    ...
    def summary(self):
        with tf.name_scope("generator_summaries"):
            ...
            generator_summaries = tf.summary.merge(
                [generator_loss_B2A_summary, generator_loss_A2B_summary, generator_loss_summary])
